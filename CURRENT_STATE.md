...
| Tests        | PASSING | 493/493 (116 web, 298 application, 21 infrastructure, 14 integrations, 44 jobs)                                                                                                                                                                                                                                        |
...
6. **Dependency patching.** `pnpm audit` reported 28 high + 1 critical. `next` was declared `^15.1.3` but the lockfile pinned 15.5.14, so 22 Next.js advisories were nothing but a stale lockfile — including a middleware/proxy bypass in App Router, which matters here because middleware does the auth redirects. (Blast radius checked: the `(protected)` layout and every feature `server.ts` also call `requireCurrentUser()`, so a bypass would have skipped the redirect, not exposed data.) Now on 15.5.24, plus `pnpm.overrides` for the fixable transitives. What remains is bundled inside Next itself or dev-only tooling.

7. **Security and reliability fixes.** Session cookies were readable from JavaScript — neither `createServerClient` call passed `cookieOptions`, so `@supabase/ssr`'s defaults applied (`httpOnly: false`, no `secure`, 400-day `maxAge`) on cookies carrying the access *and* refresh token; the usual reason to leave `httpOnly` off is a browser-side Supabase client, and `createSupabaseBrowserClient` is defined and never called. An open redirect: `sanitizeRedirectTo`'s regex blocked `//evil.com` but passed a backslash-prefixed path, which browsers normalize to an absolute URL, fired straight into `redirect()` after a successful login. And Strava/Peloton auth failures recorded nothing at all — credential decryption and provider auth ran before the `sync_job_runs` row existed and outside the try, so a revoked token left the connection `active` with a null `last_error` and the retry sweep (which selects `status = 'failed'`) had nothing to find; rides stopped importing while `/integrations` showed healthy. Also added webhook body/array caps, a uuid guard on `X-User-Id` (a malformed value returned 500 and leaked an existence oracle), and the security headers `next.config.ts` had none of.

8. **Four live bugs from a test-coverage audit**, all in code that had no tests: `computeJournalStreak` skipped a day at positive UTC offsets (a local-time cursor read back as UTC); the "Saturday long rides are slipping" rule judged the in-progress Saturday and fired on Saturday morning; the "last week has logged data but no review" nudge never checked whether data existed and fired on brand-new accounts; and three miscalculations in the dashboard's goal cards — a VO2 window counting its boundary day twice, a fat-loss pace divided by a hardcoded 4 weeks regardless of the real baseline span, and a UTC `today` fed into a user-local streak calculation, which showed a journal streak of 0 every evening after 7pm in America/Chicago. `computeGoalProgress` was unexported and untestable; it moved to `features/dashboard/helpers.ts`. Every fix is mutation-verified — reverted, watched the new test fail, restored.

9. **Also fixed:** the Apple Health sleep sync rewrote a valid 1440-minute value as 24 minutes (a `>= 1440` seconds-heuristic against an inclusive `max(1440)` validator), and `/api/account/export` silently truncated at 500 rows per table — it now asks for the largest page the data layer allows and names any section that still hit a cap.
...
**Follow-up 2 (same day, TD-035):** audited the two LLM-backed coaching features next, since neither had been covered by the earlier fitness-safety pass. `AiInsightService` and `AiWeeklyReviewService` sent Anthropic API prompts with **zero safety constraints** — no instruction against diagnosing a condition, prescribing a specific calorie/training/supplement value, or using alarmist/guilt language, which directly contradicts this project's own stated principle to treat health guidance as sensitive. Separately, `/insights` rendered AI-sourced and rule-based insights with identical styling: `PersistedInsight.sourceKind` (`"rule" | "ai"`) was already tracked in the schema and covered by orchestrator tests, but the `InsightCard` component never read it, so a user had no way to know which insights came from a deterministic, auditable rule versus unconstrained LLM output. Fixed: added explicit no-diagnosis / no-prescription / no-guilt-language / ground-in-the-provided-data rules to both prompts, added a small "AI" badge (with an explanatory tooltip) to AI-sourced insight cards, and a page-level disclaimer on `/insights`. The weekly-review AI draft panel (`AiWeeklyReviewDraft`) was already well-labeled ("AI draft — awaiting your review," "informational only," requires an explicit accept action before anything is saved) — confirmed no change needed there. No new tests: this change is prompt copy (not independently testable beyond the existing response-parsing tests, which still pass) and a UI label (this repo has no component-testing infra — verified via typecheck/lint/build instead, consistent with prior UI-only changes this session). PR #20 merged.

**Follow-up 3 (2026-08-03, TD-036):** ran a second accessibility sweep across every other feature form (cardio, recovery, body, nutrition, journal, weekly-review, settings, integrations) to check whether the strength-forms label bug (TD-033) was systemic — it wasn't; every other form already wraps its inputs in `<label>` correctly, confirmed clean. While auditing cardio/recovery numeric fields for the same investigation, found the same missing-upper-bound pattern as TD-034 in a different module: `avgHeartRate`/`maxHeartRate` (cardio) and `restingHeartRate`/`hrv` (recovery) had no `.max()` at any layer, unlike sibling fields (`rpe`, `sorenessLevel`) on the same schemas. Added generous physiological sanity bounds — heart rate ≤ 250bpm, HRV ≤ 500ms — at both client and application layers, including the cardio module's separately-defined `updateCardioSessionSchema` (not a shared base schema like most other update schemas in this codebase, so it needed its own fix). Also confirmed weekly-review's "progress one lever" strategic-decision logic (flagged as a possible gap by an earlier research pass) already gates on sleep/alcohol/confidence proxies before recommending progression — a reasonable, if imperfect, design choice; left unchanged. And confirmed no guilt/manipulation language exists anywhere in the app's own UI copy (only the new AI-prompt guardrail text itself matches a grep for those phrases). 4 new tests (261 total, up from 257). PR #21 merged.

**Follow-up 4 (2026-08-03, TD-037):** the missing-sanity-bound pattern (TD-034, TD-036) showed up a third time in `body-metric.ts` — `weightLb`/`weightKg`/`waistIn`/`waistCm`/`waistHipIn`/`waistGutIn`/`muscleMassLb`/`muscleMassKg`/`boneMassLb`/`boneMassKg`/`fatFreeMassLb`/`fatFreeMassKg`/`visceralFatIndex` all had no upper bound. This one is more consequential than the cardio/recovery instances: `weightLb`/`weightKg` feed `NutritionTargetService`'s calorie/protein calculation directly (the same service fixed for TD-030), so a garbage weight — a typo, or a unit-conversion bug in an imported Withings/Apple Health payload — would silently produce a wrong calorie target with nothing catching it. Added bounds at both the client (`apps/web/src/features/body/form-schema.ts`) and application layers: weight ≤ 1000lb/500kg, waist ≤ 120in/300cm, muscle/bone/fat-free mass ≤ 1000lb/500kg, visceral fat index ≤ 60. Also found and fixed a related gap while in the area: `alcoholCount` on both `nutrition-log-schemas.ts` and `recovery-checkin.ts` had a `.max()` at the web form layer but not at the application layer — meaning any non-web caller (a future API, a bulk import) could bypass it entirely. Matched the application-layer bound to each module's existing web-layer bound (≤ 50 for nutrition log, ≤ 20 for recovery). 4 new tests (265 total, up from 261 — 2 for body-metric bounds, 1 each for the two alcoholCount fixes). PR #22 merged (needed a manual conflict resolution after a local commit-authorship amend diverged the branch's history from `main` — resolved by merging `origin/main` in and taking the branch's version, which was a strict superset of what was already on `main`; verified with `git diff` before resolving and re-ran the full check suite after).

**Follow-up 5 (2026-08-03, TD-038):** closed out the missing-sanity-bound sweep with the last two instances found by checking every remaining module with numeric fields. (1) `weekly_reviews.summary` is user-editable via `manualOverrides` (the "Refresh from logs" / manual entry fields on `/weekly-review`) but had **no bounds at all** at the application layer for `averageWeightLb`/`waistIn`/`liftsCompleted`/`ridesCompleted`/`zone2Minutes`/`sleepAverageHours`/`alcoholTotal` — not even a minimum, so a negative weight would have passed — while the web form layer already had good bounds on everything except `averageWeightLb`/`waistIn`. (2) `daily_activity_metrics` (`steps`/`vo2Max`/`restingHeartRate`/`exerciseMinutes`/`activeEnergyKcal`) is populated entirely by the Apple Health webhook with no manual form at all, and had no upper bound anywhere — the realistic failure mode there is a malformed/buggy import payload, not a typo. Added matching bounds: weekly-review summary now mirrors its own web-form bounds at the application layer too; daily-activity-metric gets steps ≤ 200000, vo2Max ≤ 100, restingHeartRate ≤ 250, exerciseMinutes ≤ 1440 (minutes/day), activeEnergyKcal ≤ 20000. Checked the remaining modules (journal, supplements) and found no unbounded numeric fields — the sweep is now complete across the whole schema surface. 4 new tests (269 total, up from 265).
