...
| Tests        | PASSING | 228/228 (47 web, 139 application, 14 integrations, 28 jobs)                                                                                                                                                                                                                                        |
...
3. User: apply migrations `20260721190000_create_auth_rate_limit_attempts.sql`, `20260722170000_create_exercise_muscle_group_overrides.sql`, and `20260722180000_add_target_weight_and_template_schedule.sql` to the live Supabase project (`supabase db push`)
...
5. User: once the migration above is applied, set a target weight/date under Settings → Training Goals, and create/pin your M/W/F strength templates with a scheduled day under `/strength` — the underlying capability (TD-027) is now built, but the actual numbers/templates are personal data this container has no way to enter on your behalf
...
1. **Numeric fat-loss target** — `targetWeightLb`/`targetDate` added to the profile (migration `20260722180000`, not yet applied to the live project — see note above), editable under Settings → Training Goals. The dashboard's fat-loss goal card now shows real progress-to-target ("14.0lb to go — on pace for 2027-03-01") and an at/below-target state when a target is set, falling back to the previous trend-only heuristic when it isn't — fully backward compatible for users who never set a target.
