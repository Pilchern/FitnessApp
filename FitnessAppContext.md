...
Resolved 2026-08-02: false personal-record badges from an uncapped 1RM-estimate formula (TD-031 — capped the rep term used in `setScore()` at 12), recovery coaching ignoring soreness (TD-032 — added a soreness check ahead of the readiness checks in `getRecoveryCoachingSuggestion()`), and 12 missing `aria-label`s on the strength logging/template/exercise-classification forms (TD-033).
...
- **Current coverage:** 253 unit/integration tests across application, integrations, jobs, and web layers
