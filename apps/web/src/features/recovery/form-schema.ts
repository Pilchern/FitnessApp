...
export const recoveryCheckinFormSchema = z.object({
  ...
  readinessLevel: parseOptionalNumber(
  ...
  sorenessLevel: parseOptionalNumber(
    z.number().int().min(1).max(10),
    "Soreness",
  ),
  ...
  restingHeartRate: parseOptionalNumber(
    z.number().int().min(0).max(250),
    "Resting heart rate",
  ),
  hrv: parseOptionalNumber(z.number().min(0).max(500), "HRV"),
  coldPlungeCompleted: parseBooleanCheckbox,
  notes: optionalString,
});

export type RecoveryCheckinFormInput = z.infer<
  typeof recoveryCheckinFormSchema
>;
